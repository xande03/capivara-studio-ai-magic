...
export default function ConverterPage() {
  ...
  const handleAddPdfImage = (base64: string) => setPdfImages((prev) => [...prev, base64]);

  const handleConvertToPdf = () => {
    if (pdfImages.length === 0) { toast({ title: "Adicione pelo menos uma imagem", variant: "destructive" }); return; }
    setPdfLoading(true);
    try {
      const pdf = new jsPDF();
  ...
  const handlePdfUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type === "application/pdf") { setPdfFile(file); setWordText(""); }
    else toast({ title: "Selecione um arquivo PDF válido", variant: "destructive" });
  };
    ...
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        if (pageText.trim()) {
          ...
          try {
            const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/document-process`, {
              method: "POST",
            ...
            if (response.status === 402) {
              ...
              fullText += `--- Página ${i} ---\n[OCR indisponível — créditos insuficientes]\n\n`;
            } else if (response.ok) {
  ...
  const handleDownloadWord = async () => {
    ...
    } catch { toast({ title: "Erro ao gerar Word", variant: "destructive" }); }
  };

  const handleScan = async () => {
    if (!scanImage) { toast({ title: "Selecione uma imagem", variant: "destructive" }); return; }
    setScanLoading(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/document-process`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY}` },
        body: JSON.stringify({ action: "ocr", imageBase64: scanImage }),
      });
      if (response.status === 402) { setCreditsExhausted(true); toast({ title: "Créditos insuficientes para OCR", variant: "destructive" }); setScanLoading(false); return; }
      if (!response.ok) throw new Error("Erro no servidor");
      const data = await response.json();
      setScanResult(data.text || "Nenhum texto encontrado.");
    } catch { toast({ title: "Erro ao escanear", variant: "destructive" }); }
  ...
  const handleDownloadScanPdf = () => {
    ...
    pdf.setFontSize(12); pdf.text(lines, 20, 20);
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs md:text-sm text-muted-foreground">Converta imagens em PDF, PDF em Word e escaneie documentos</p>
        </div>
      </div>

      <CreditsBanner visible={creditsExhausted} message="Créditos insuficientes para funcionalidades de OCR/IA. Conversão de imagem para PDF continua funcionando normalmente." />

      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="grid grid-cols-3 w-full">
          <TabsTrigger value="img2pdf" className="flex items-center gap-1 md:gap-2 text-xs md:text-sm"><Image className="w-4 h-4" /> <span className="hidden sm:inline">Imagem →</span> PDF</TabsTrigger>
          <TabsTrigger value="pdf2word" className="flex items-center gap-1 md:gap-2 text-xs md:text-sm"><FileDown className="w-4 h-4" /> <span className="hidden sm:inline">PDF →</span> Word</TabsTrigger>
          <TabsTrigger value="scan" className="flex items-center gap-1 md:gap-2 text-xs md:text-sm"><ScanLine className="w-4 h-4" /> Escanear</TabsTrigger>
        </TabsList>

        <TabsContent value="img2pdf">
          <div className="glass-card rounded-xl p-4 md:p-5 space-y-4">
            <h3 className="text-sm font-semibold text-foreground uppercase tracking-wide">Adicionar Imagens</h3>
            <ImageUploader onImageSelect={handleAddPdfImage} currentImage="" onClear={() => {}} label="Clique ou arraste imagens" />
            {pdfImages.length > 0 && (
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground">{pdfImages.length} imagem(ns) adicionada(s)</p>
                <div className="flex gap-2 flex-wrap">
                  {pdfImages.map((img, i) => (
                    <div key={i} className="w-16 h-16 rounded border border-border overflow-hidden cursor-pointer" onClick={() => setLightbox(img)}>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Button size="sm" variant="outline" onClick={async () => {
                        const { Document, Packer, Paragraph, TextRun } = await import("docx");
                        const { saveAs } = await import("file-saver");
                        const doc = new Document({ sections: [{ children: scanResult.split("\n").map((line) => new Paragraph({ children: [new TextRun({ text: line, size: 24 })] })) }] });
                        const blob = await Packer.toBlob(doc); saveAs(blob, `scan-${Date.now()}.docx`); toast({ title: "Arquivo .docx salvo!" });
                      }}><Download className="w-4 h-4 mr-2" /> Salvar como .docx</Button>
                    </div>
                  </div>
                )}
              </>
